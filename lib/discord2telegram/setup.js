...
function setup(logger, dcBot, tgBot, messageMap, bridgeMap, settings) {
	...
	dcBot.on("message", async (message) => {
		...
		if (bridge !== undefined) {
			...
			message.reply(
				"Mercury is a private bot which is an instance of a TediCross bot, bridging a chat in Telegram with one in Discord. "
				...
				+ 
				+
				+"If you need any other assitance such as Wallet/Masternode support please contact a human :)"
			);
		}
	});
